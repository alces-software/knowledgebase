...
curl $FILES_URL/hosts | envsubst "$_ALCES_KEYS" > /etc/hosts

rm -rf /etc/yum.repos.d/*.repo
curl $FILES_URL/yum.local | envsubst "$_ALCES_KEYS" > /etc/yum.repos.d/cluster.repo

yum -y install ntp
curl $FILES_URL/ntp | envsubst "$_ALCES_KEYS" > /etc/ntp.conf
systemctl enable ntpd
systemctl restart ntpd

mkdir -m 0700 /root/.ssh
curl $FILES_URL/authorized_keys | envsubst "$_ALCES_KEYS" > /root/.ssh/authorized_keys
